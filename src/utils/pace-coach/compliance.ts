...
export async function calculateWeeklyCompliance(
  ...
  const loggedDaysSet = new Set(
    macroLogs.map((log) => new Date(log.date).toDateString()),
  );
  const totalCalories = macroLogs.reduce((sum, log) => sum + log.calories, 0);
  const compliantDays = macroLogs.filter(
    (log) => Math.abs(log.calories - targetCalories) <= 100,
  ).length;
  const loggedDays = loggedDaysSet.size;
  const complianceScore = loggedDays > 0 ? compliantDays / loggedDays : 0;
  const isCompliant =
    complianceScore >= PACE_COACH_COMPLIANCE_THRESHOLD &&
    loggedDays >= PACE_COACH_MIN_LOGGED_DAYS;
...
export async function processWeeklyMetricsForUsers(
  ...
  for (const userId of userIds) {
    ...
    if (
      ...
      continue;
    try {
      const metrics = await calculateWeeklyCompliance(
        userId,
        priorIsoWeek,
        targetCalories,
      );
