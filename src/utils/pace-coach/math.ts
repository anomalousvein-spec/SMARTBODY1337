...
export function calculateSuggestedIntake(params: {
  ...
  if (currentWeight && currentWeight > 0) {
    ...
    const result = calculatePercentageLoss(
      currentWeight, 'lbs', effectivePercentage, currentTDEE,
      bmr || (gender === 'male' ? BMR_FLOOR_MALE : BMR_FLOOR_FEMALE), gender || 'female'
    );
    ...
    if (lastSuggestedIntake) {
      ...
      if (Math.abs(diff) > MAX_STEP) suggestion = lastSuggestedIntake + (diff > 0 ? MAX_STEP : -MAX_STEP);
    }
    return suggestion;
  }
...
export function calculateAverageIntakeFromLogs(
  ...
  if (macroLogs.length === 0) return { averageCalories: 0, hasSufficientData: false, daysLogged: 0 };
  const sortedLogs = [...macroLogs].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
  ...
  if (recentLogs.length === 0) return { averageCalories: 0, hasSufficientData: false, daysLogged: 0 };
  const averageCalories = Math.round(recentLogs.reduce((sum, log) => sum + log.calories, 0) / recentLogs.length);
  return { averageCalories, hasSufficientData: recentLogs.length >= 10, daysLogged: recentLogs.length };
}
