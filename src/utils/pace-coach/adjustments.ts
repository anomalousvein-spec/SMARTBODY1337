...
import {
  PACE_COACH_ADJUSTMENT_STEP,
  PACE_COACH_MAX_ADJUSTMENT_STEP,
  PACE_COACH_RATE_TOLERANCE_LBS,
...
export function determineAdjustment(params: {
  ...
  const {
    trendRateLbsPerWeek,
    goalRateLbsPerWeek,
    adjustmentStep = PACE_COACH_ADJUSTMENT_STEP,
  } = params;
  const tol = PACE_COACH_RATE_TOLERANCE_LBS;
  if (goalRateLbsPerWeek < 0) {
    if (trendRateLbsPerWeek > goalRateLbsPerWeek + tol)
    ...
    return {
      adjustmentKcal:
        trendRateLbsPerWeek < 0 ? adjustmentStep : -adjustmentStep,
      reason: trendRateLbsPerWeek < 0 ? "TOO_FAST" : "TOO_SLOW",
    };
  }
...
export async function calculateEnhancedCheckInSuggestion(params: {
  ...
  if (!trend.canCalculate)
    return {
      ...
      tier: trend.tier,
      adjustmentReason: trend.reason || "INSUFFICIENT_DATA",
      canAdjust: false,
      holdReason: trend.reason,
    };
  const metrics = await db.weekly_metrics
    .where("[user_id+iso_week]")
    .equals([userId, currentIsoWeek])
    .first();
  if (metrics && !metrics.adjustment_eligible)
    return {
      suggestedIntake: calculateSuggestedIntake({
        currentTDEE,
        goalLbsPerWeek: Math.abs(goalRateLbsPerWeek),
      ...
      adjustmentKcal: 0,
      trendRateLbsPerWeek: trend.trendRateLbsPerWeek,
      tier: trend.tier,
      adjustmentReason: "COMPLIANCE_HOLD",
      canAdjust: false,
      holdReason: metrics.hold_reason || undefined,
    };
  const adj = determineAdjustment({
    trendRateLbsPerWeek: trend.trendRateLbsPerWeek,
    goalRateLbsPerWeek,
    currentTarget,
  });
  ...
  if (Math.abs(diff) > PACE_COACH_MAX_ADJUSTMENT_STEP)
    newTarget =
      currentTarget +
      (diff > 0
        ? PACE_COACH_MAX_ADJUSTMENT_STEP
  ...
  return {
    ...
    adjustmentKcal: adj.adjustmentKcal,
    trendRateLbsPerWeek: trend.trendRateLbsPerWeek,
    tier: trend.tier,
...
export async function processEnhancedCheckIn(
  ...
  const profile = await db.user_profiles
    ...
    .equals(userId)
    .first();
  const settings = await db.tdee_settings
    .where("user_id")
    .equals(userId)
    .first();
  if (!settings) throw new Error("User TDEE settings not found");
  const currentTarget =
    profile?.current_target_calories || settings.cuttingCalories || 2000;
  const goalRate =
  ...
  if (profile) {
    const metrics = await db.weekly_metrics
      .where("[user_id+iso_week]")
      .equals([userId, isoWeek])
      .first();
    const newStreak = metrics?.adjustment_eligible
      ? profile.consecutive_compliant_weeks + 1
      : metrics && !metrics.adjustment_eligible
        ? 0
        : profile.consecutive_compliant_weeks;
  ...
  return {
    newTarget,
    adjustmentAmount: adjKcal,
    trendRate: trend.trendRateLbsPerWeek,
    triggers: await evaluateSmartTriggers(userId),
  };
}
