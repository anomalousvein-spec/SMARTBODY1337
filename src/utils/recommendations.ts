...
export function analyzeWeightTrend(
  ...
  const recentAvg =
    recentWeek.reduce((sum, w) => sum + w.weight, 0) / recentWeek.length;
  const previousAvg =
  ...
  if (
    Math.abs(actualLossPerWeek) < PLATEAU_THRESHOLD_LBS &&
    expectedLossPerWeek > 0.5
  ) {
    recommendations.push({
      ...
      severity: "warning",
      action:
        "Consider reducing daily calories by 100-200 or increasing activity level.",
  ...
  if (actualLossPerWeek > RAPID_LOSS_THRESHOLD_LBS_PER_WEEK) {
    recommendations.push({
      ...
      severity: "critical",
      action:
        "Increase your daily calorie intake by 200-300 calories to slow down weight loss to a safer rate.",
      timestamp: new Date().toISOString(),
    });
  }

  // Detect stall (no progress for extended period)
  const twoWeeksAgo = sortedWeights.find(
    (w) =>
      new Date(w.date) <=
      new Date(Date.now() - STALL_DETECTION_DAYS * 24 * 60 * 60 * 1000),
  );

  if (
    twoWeeksAgo &&
    Math.abs(twoWeeksAgo.weight - recentAvg) < STALL_THRESHOLD_LBS
  ) {
    recommendations.push({
      id: `stall-${Date.now()}`,
      type: "stall_detected",
      ...
      severity: "warning",
      action:
        "Try a diet break (eat at maintenance for 1 week) or reassess your calorie tracking accuracy.",
      timestamp: new Date().toISOString(),
    });
  }
  ...
  if (deviation < -WEIGHT_DEVIATION_THRESHOLD && actualLossPerWeek < 0) {
    ...
    deviation > WEIGHT_DEVIATION_THRESHOLD &&
    actualLossPerWeek > 0 &&
    actualLossPerWeek < RAPID_LOSS_THRESHOLD_LBS_PER_WEEK
  ) {
...
export function analyzeMacroIntake(
  ...
  const avgCalories =
    recentMacros.reduce((sum, m) => sum + m.calories, 0) / recentMacros.length;
  const avgProtein =
    recentMacros.reduce((sum, m) => sum + m.protein, 0) / recentMacros.length;
  ...
  const calorieDeviation =
    ((avgCalories - targetCalories) / targetCalories) * 100;
  ...
  if (calorieDeviation > CALORIE_OVER_DEVIATION_PERCENT) {
    recommendations.push({
      ...
      severity: "warning",
      action:
        "Review your food logging accuracy and portion sizes. Consider meal prepping for better control.",
      timestamp: new Date().toISOString(),
    });
    ...
    recommendations.push({
      ...
      severity: "warning",
      action:
        "Ensure you're eating enough to support your metabolism and energy levels.",
      timestamp: new Date().toISOString(),
    });
  }
  ...
  if (avgProtein < minProteinTarget) {
    recommendations.push({
      ...
      severity: "info",
      action:
        "Aim for ${MIN_PROTEIN_PER_LB}-${MAX_PROTEIN_PER_LB}g of protein per pound of bodyweight to preserve muscle mass during weight loss.",
      timestamp: new Date().toISOString(),
    });
  }
