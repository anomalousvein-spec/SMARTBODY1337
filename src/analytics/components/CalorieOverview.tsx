...
/**
 * Optimized component for displaying maintenance and cutting calorie targets
 */
export const CalorieOverview = memo(({ metrics }: CalorieOverviewProps) => {
  return (
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="text-center p-3 bg-theme-bg-tertiary/50 rounded-lg">
          ...
          <div className="text-xl font-bold text-theme-text-primary">
            {metrics?.maintenanceCalories ?? 2000}
          </div>
      ...
      {metrics?.todayMacros && (
        <div className="mt-4 pt-4 border-t border-white/5">
          <div className="flex justify-between items-center">
            <span className="text-sm text-theme-text-tertiary">Today's Intake</span>
            <span className="text-lg font-bold text-theme-text-primary">
              {metrics.todayMacros.calories} cal
            </span>
          </div>
          <div className="flex justify-between items-center mt-2">
            <span className="text-sm text-theme-text-tertiary">Protein</span>
      ...
      {!metrics?.todayMacros && (
        <div className="mt-4 p-3 bg-yellow-500/10 rounded-lg flex items-center gap-2">
          <AlertCircle className="w-4 h-4 text-yellow-400" aria-hidden="true" />
          <span className="text-sm text-yellow-700 dark:text-yellow-400">No macros logged today</span>
        </div>
      )}
    </motion.div>
  );
...
CalorieOverview.displayName = 'CalorieOverview';
