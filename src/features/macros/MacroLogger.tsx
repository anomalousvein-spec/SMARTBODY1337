...
export function MacroLogger({ userId, onMacroLogged }: MacroLoggerProps) {
  ...
  const handleSubmit = useCallback(async (e: React.FormEvent) => {
    ...
    if (!protVal.valid) { setError(protVal.error!); return; }
  ...
  return (
    <Card className="card-hover">
      <h2 className="text-xl font-bold text-theme-text-primary mb-4">Log Macros</h2>

      <form onSubmit={handleSubmit} className="space-y-4" noValidate>
        <InputField label="Date" type="date" value={date} onChange={setDate} required />

        <div className="grid grid-cols-2 gap-4">
          <InputField label="Calories" type="number" value={calories} onChange={setCalories} placeholder="0" required />
          <InputField label="Protein (g)" type="number" value={protein} onChange={setProtein} placeholder="0" />
        </div>

        <div className="grid grid-cols-2 gap-4">
          <InputField label="Carbs (g)" type="number" value={carbs} onChange={setCarbs} placeholder="0" />
          <InputField label="Fats (g)" type="number" value={fats} onChange={setFats} placeholder="0" />
        </div>

        <TextAreaField label="Notes (optional)" value={notes} onChange={setNotes} placeholder="What did you eat?" rows={2} />
