...
export function WaistRatioDisplay() {
  ...
  const { measurements: waistEntries, isLoading: waistLoading } =
  ...
  if (waistLoading || settingsLoading)
    return (
  ...
  if (!analytics)
    return (
      <Card className="card-hover">
        <p className="text-center text-theme-text-tertiary py-8 font-medium">
          Log your first waist measurement and set your height in Profile to see
          your ratio!
        </p>
      </Card>
    );
    ...
    <Card className="card-hover">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="p-4 bg-theme-bg-tertiary/40 border border-white/5 rounded-xl">
          ...
          <p className="text-2xl font-bold text-theme-text-primary">
            ...
            <span className="text-sm font-normal text-theme-text-tertiary ml-1">
              {analytics.latestUnit}
            </span>
          </p>
          <p className="text-[10px] font-bold text-theme-text-tertiary uppercase tracking-wide mt-1">
            {new Date(analytics.latestDate).toLocaleDateString()}
          </p>
        </div>
        <div
          className={`p-4 rounded-xl border border-white/5 ${analytics.healthInfo.category === "Healthy" ? "bg-success/10" : "bg-theme-bg-tertiary/40"}`}
        >
          <p className="text-[10px] font-black uppercase tracking-widest text-theme-text-tertiary mb-1">
            Ratio
          </p>
          ...
          <p
            className={`text-[10px] font-black uppercase tracking-widest ${analytics.healthInfo.color} mt-1`}
          >
            {analytics.healthInfo.category}
          </p>
        </div>
        <div
          className={`p-4 rounded-xl border border-white/5 ${isPositiveProgress ? "bg-success/10" : "bg-error/10"}`}
        >
          <p className="text-[10px] font-black uppercase tracking-widest text-theme-text-tertiary mb-1">
            Total Change
          </p>
          <p
            className={`text-2xl font-bold ${isPositiveProgress ? "text-success" : "text-error"}`}
          >
            {isPositiveProgress ? "-" : "+"}
            {Math.abs(analytics.totalChangeCm).toFixed(1)}
      ...
      <div className="mt-4 p-4 bg-theme-accent/10 border border-theme-accent/10 rounded-xl">
        <p className="text-sm text-theme-text-secondary">
          <span className="text-[10px] font-black uppercase tracking-widest text-theme-accent mr-2">
            Insight
          </span>{" "}
          {analytics.healthInfo.description}
        </p>
      </div>
    </Card>
  );
