...
export function AdvancedResultsView({
  ...
  const whtCategory = getWaistToHeightCategory(
    advancedResults.waistToHeightRatio,
  );
    ...
    <div className="space-y-6">
      <div className="p-4 bg-theme-bg-tertiary/30 border border-white/5 rounded-xl">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              <div className="bg-theme-bg-tertiary/40 border border-white/5 rounded-lg p-3">
                <p className="text-[9px] font-black uppercase tracking-widest text-theme-text-tertiary mb-1">
                  Estimated Body Fat
                </p>
                <p className="text-lg font-bold text-theme-accent">
                  {advancedResults.avgBodyFat.toFixed(1)}%{" "}
                  <span className="text-[10px] font-normal text-theme-text-tertiary">
                    (Composite)
                  </span>
                </p>
              </div>
              <div className="bg-theme-bg-tertiary/40 border border-white/5 rounded-lg p-3">
                ...
                <p className="text-lg font-bold text-success">
                  {(advancedResults.leanBodyMass * KG_TO_LBS).toFixed(1)} lbs
                </p>
              </div>
              <div className="bg-theme-bg-tertiary/40 border border-white/5 rounded-lg p-3">
                <p className="text-[9px] font-black uppercase tracking-widest text-theme-text-tertiary mb-1">
                ...
                <p
                  className={`text-lg font-bold ${whtCategory.color.includes(" ") ? whtCategory.color.split(" ")[0] : whtCategory.color}`}
                >
                  {advancedResults.waistToHeightRatio.toFixed(2)}
                </p>
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                <div className="bg-theme-bg-tertiary/40 border border-white/5 rounded-lg p-3">
                  ...
                  <p className="text-lg font-bold text-theme-accent">
                    {Math.round(advancedResults.bmr)}{" "}
                ...
                <div className="bg-theme-bg-tertiary/40 border border-white/5 rounded-lg p-3">
                  ...
                  <p className="text-lg font-bold text-orange-400">
                    {Math.round(macroTargets.proteinMin)}-
                    {Math.round(macroTargets.proteinMax)}g
                  </p>
                  <p className="text-[10px] text-theme-text-tertiary">
                    Optimized for Lean Mass
                  </p>
                </div>
                <div className="bg-theme-bg-tertiary/40 border border-white/5 rounded-lg p-3">
                  <p className="text-[9px] font-black uppercase tracking-widest text-theme-text-tertiary mb-1">
                  ...
                  <p className="text-lg font-bold text-yellow-400">
                    {Math.round(macroTargets.fatMin)}g
                  </p>
                  <p className="text-[10px] text-theme-text-tertiary">
                    Hormonal Floor
                  </p>
        ...
        <p className="text-xs text-theme-text-tertiary mt-4 italic">
          “Meeting your Protein and Fat floors ensures muscle retention and
          hormonal health. Adjust Carbs based on your daily activity.”
        </p>
      </div>
