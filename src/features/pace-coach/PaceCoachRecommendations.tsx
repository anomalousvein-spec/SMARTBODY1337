...
import { Sparkles, Target, TrendingUp, TrendingDown, CheckCircle, ShieldCheck, RefreshCw } from 'lucide-react';
import { Metrics } from '../../hooks/useMetrics';
import { Card } from '../../components';
import { useTDEESettings } from '../../hooks/useTDEESettings';
import { useApp } from '../../context/AppContext';
import { calculateBMR, calculateTDEE } from '../../utils/calculations';
import { calculatePercentageLoss, SLIDER_CONFIG } from '../../utils/percentageLoss';
...
export const PaceCoachRecommendations = ({ metrics }: PaceCoachRecommendationsProps) => {
  ...
  const handleResetToFormula = async () => {
    if (!settings) return;
    try {
      const weightValue = settings.currentWeight || metrics.latestWeight?.weight || 0;
      ...
      const heightCm = settings.heightUnit === 'in' ? settings.height * INCHES_TO_CM : settings.height;
  ...
  const isApplied =
    Math.round(metrics.paceCoachMaintenance) === Math.round(metrics.maintenanceCalories) &&
    Math.round(metrics.paceCoachTarget) === Math.round(metrics.cuttingCalories);

  const getConfidence = (count: number) => {
    if (count < 3) return { label: "Low Confidence", color: "text-orange-400", bg: "bg-orange-400/10" };
    if (count < 5) return { label: "Medium Confidence", color: "text-yellow-400", bg: "bg-yellow-400/10" };
    return { label: "High Confidence", color: "text-green-400", bg: "bg-green-400/10" };
  };
      ...
      <Card className="card-hover overflow-hidden relative group border-theme-accent/30">
        ...
        <div className="relative">
          <div className="flex items-start justify-between mb-5">
            ...
            <div className={`px-2 py-1 rounded-lg ${confidence.bg} border border-white/5 flex items-center gap-1.5`}>
              <div className={`w-1.5 h-1.5 rounded-full ${confidence.color.replace('text', 'bg')} animate-pulse`} />
              <span className={`text-[9px] font-black uppercase tracking-tighter ${confidence.color}`}>
                {confidence.label}
              </span>
            </div>
          </div>
