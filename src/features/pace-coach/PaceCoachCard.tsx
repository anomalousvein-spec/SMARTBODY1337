...
export function PaceCoachCard() {
  ...
  const getTrendInfo = () => {
    if (!lastCheckIn || !lastCheckIn.weightSlopeLbsPerDay) {
      return { icon: Minus, color: 'text-theme-text-tertiary', bgColor: 'bg-theme-bg-tertiary/50', label: 'Insufficient data', description: '' };
    }
    ...
    if (Math.abs(slope) < 0.1) {
      return { icon: Minus, color: 'text-theme-text-secondary', bgColor: 'bg-theme-bg-tertiary/50', label: 'Weight stable', description: `±${lbsPerWeek.toFixed(1)} lbs/week` };
    } else if (slope < 0) {
      return { icon: TrendingDown, color: 'text-green-500', bgColor: 'bg-green-500/10', label: 'Losing weight', description: `${lbsPerWeek.toFixed(1)} lbs/week` };
    } else {
      return { icon: TrendingUp, color: 'text-orange-500', bgColor: 'bg-orange-500/10', label: 'Gaining weight', description: `${lbsPerWeek.toFixed(1)} lbs/week` };
    }
  };
      ...
      <div className="relative">
        <div className="flex items-center justify-between mb-5">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-lg font-bold text-theme-text-primary">Pace Coach</h3>
              <p className="text-[10px] text-theme-text-tertiary uppercase tracking-wider font-semibold">Smart Calibration</p>
            </div>
          </div>
          {!showCheckInForm && (
            <button 
              ...
              <div className="space-y-4">
                <div className="bg-gradient-to-br from-theme-accent/10 to-theme-accent/5 p-5 rounded-2xl border border-theme-accent/20">
                  <div className="flex items-start gap-3 mb-4">
                    ...
                    <div className="flex-1">
                      <p className="text-sm font-bold text-theme-text-primary">Calibrating to Your Metabolism</p>
                      <p className="text-xs text-theme-text-secondary mt-1 leading-relaxed">
                        Pace Coach learns from your actual data to provide personalized recommendations.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-[10px] text-theme-text-tertiary text-center pt-1">
                      {MIN_CHECKINS_FOR_METABOLISM - checkInCount} more to unlock full insights
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2 px-3 py-2.5 bg-theme-bg-tertiary/30 rounded-xl border border-theme-bg-border/50">
                  ...
                  <p className="text-[10px] text-theme-text-tertiary leading-relaxed">
                    <strong className="text-theme-text-secondary">Tip:</strong> Weigh in consistently at the same time daily for best results
                  </p>
                </div>
              </div>
            ) : (
              ...
              <div className="space-y-5">
                ...
                <div className="relative overflow-hidden">
                  ...
                  <div className="text-center p-6 bg-gradient-to-b from-theme-accent/15 via-theme-accent/5 to-transparent rounded-2xl border border-theme-accent/20 relative">
                    <div className="flex items-center justify-center gap-1.5 mb-2">
                      ...
                      <p className="text-[10px] text-theme-text-tertiary uppercase tracking-widest font-bold">Recommended Intake</p>
                    </div>
                    <div className="text-5xl font-black text-theme-text-primary tracking-tight">
                      {settings.lastSuggestedIntake}
                      <span className="text-base font-medium text-theme-text-tertiary ml-1.5">kcal/day</span>
                    </div>
                    {lastCheckIn && (
                ...
                {lastCheckIn && (
                  <div className="grid grid-cols-2 gap-3">
                    {/* Trend Card */}
                    <div className={`${trendInfo.bgColor} rounded-2xl p-4 border border-theme-bg-border/50`}>
                      <div className="flex items-center gap-2 mb-2">
                        <div className={`p-1.5 rounded-lg ${trendInfo.bgColor} ${trendInfo.color}`}>
                          <TrendIcon className="w-3.5 h-3.5" />
                        </div>
                        <span className="text-[10px] font-bold text-theme-text-tertiary uppercase tracking-wide">Trend</span>
                      </div>
                      <p className="text-sm font-bold text-theme-text-primary">{trendInfo.label}</p>
                      {trendInfo.description && (
                        <p className="text-xs text-theme-text-secondary mt-0.5">{trendInfo.description}</p>
                      )}
                    </div>
                    ...
                    <div className="bg-theme-bg-tertiary/30 rounded-2xl p-4 border border-theme-bg-border/50">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className="text-[10px] font-bold text-theme-text-tertiary uppercase tracking-wide">Est. TDEE</span>
                      </div>
                      <p className="text-sm font-bold text-theme-text-primary">{Math.round(lastCheckIn.calculatedTDEE)}</p>
                      <p className="text-xs text-theme-text-secondary mt-0.5">calories/day</p>
                    </div>
                  </div>
                )}
                ...
                <div className="bg-gradient-to-br from-theme-bg-tertiary/50 to-theme-bg-tertiary/20 rounded-2xl p-4 border border-theme-bg-border/50">
                  <div className="flex items-start gap-3 mb-3">
                    ...
                    <div className="flex-1">
                      <p className="text-sm font-semibold text-theme-text-primary">Powered by Your Data</p>
                      <p className="text-xs text-theme-text-tertiary mt-1 leading-relaxed">
                        Your target is based on your <strong className="text-theme-text-secondary">actual weight trend</strong> from {checkInCount} check-ins, not generic formulas.
                      </p>
                    </div>
                  </div>
