...
export function WeightAnalytics() {
  ...
  const analytics = useMemo(() => {
    ...
    return {
      ...
      projectedWeeks:
        targetWeight && weeklyAvgLoss > 0
          ? Math.ceil((currentWeight - targetWeight) / weeklyAvgLoss)
          : null,
      entriesCount: weights.length,
    };
  }, [weights, targetWeight]);

  if (weightsLoading || settingsLoading)
    return (
    ...
    <Card className="card-hover">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
        <div className="p-4 bg-theme-bg-tertiary/40 border border-white/5 rounded-xl">
          ...
          <p className="text-2xl font-bold text-theme-text-primary">
            ...
            <span className="text-xs font-normal text-theme-text-tertiary ml-1 uppercase">
              lbs
            </span>
          </p>
        </div>
        ...
        >
          ...
          >
            {isPositiveProgress ? "-" : "+"}
            {Math.abs(analytics.totalLoss).toFixed(1)}
            <span className="text-xs font-normal ml-1 uppercase">lbs</span>
          </p>
        </div>
        <div className="p-4 bg-theme-bg-tertiary/40 border border-white/5 rounded-xl">
          <p className="text-[10px] font-black uppercase tracking-widest text-theme-text-tertiary mb-1">
      ...
      {targetWeight && analytics.projectedWeeks !== null && (
        <div className="mt-4 p-4 bg-theme-accent/10 border border-theme-accent/10 rounded-xl">
          <p className="text-[10px] font-black uppercase tracking-widest text-theme-accent mb-1">
            Projected Weeks to Goal ({targetWeight} lbs)
          </p>
          <p className="text-2xl font-bold text-theme-accent">
            {analytics.projectedWeeks}
            <span className="text-sm font-normal text-theme-text-tertiary ml-1 uppercase">
              weeks
            </span>
          </p>
        </div>
